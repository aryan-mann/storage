...
;;; for lexical tokens of the LETREC-LCPI programming language,
...
;;; LETREC-LCPI has tokens of fifteen kinds:
...
(define-library (LETREC-LCPI tokens)
  (export token? numeral-token minus-sign open-parenthesis comma
          ...
          ;; [Aryan]
          cons-token null?-token car-token cdr-token empty-list-token
          ;; [Aryan]
          letrec-token)
  (import (scheme base)
          (utilities eopl))
