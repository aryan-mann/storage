...
;;; Tests for the (LETREC scanner) library
...
(suite peek ()
  ...
  (test peek:proc-token
    ((scanner (make-character-source " proc !")) 'peek)
    1 (token?)
    (lambda (result)
      (cases token result
        (proc-token () #t)
  ...
  (test peek:letrec-token
    ((scanner (make-character-source "letrec%comment")) 'peek)
    1 (token?)
    (lambda (result)
      (cases token result
        (letrec-token () #t)
