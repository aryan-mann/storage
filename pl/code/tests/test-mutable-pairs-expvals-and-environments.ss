...
;;; Tests for the (MUTABLE-PAIRS expvals-and-environments) library
...
(suite expval? ()
  ...
  (test expval?:mutpair-val
    (begin
      (initialize-store!)
      (expval? (mutpair-val (a-pair (newref (num-val 43))
                                    (newref (num-val 44))))))
    1 (true?))
...
(suite expval->proc ((error-message
                       (begin
                         ...
                         (let ((message (open-output-string)))
                           (display "In expval->proc: undefined for expressed value "
                                    message)
                           (write (mutpair-val (a-pair (newref (num-val 38))
                                                       (newref (num-val 39))))
                                  message)
                           (newline message)
                           (let ((result (get-output-string message)))
  ...
  (test expval->proc:no
    (guard (object
            ...
            (else 'fail))
      (initialize-store!)
      (expval->proc (mutpair-val (a-pair (newref (num-val 38))
                                         (newref (num-val 39))))))
    1 (symbol?)
    (match eq? 'pass)))

(suite expval->mutpair ((error-message
                          (begin
                            (initialize-store!)
                            (let ((message (open-output-string)))
                              (display "In expval->mutpair: undefined for expressed value "
                                       message)
                              (write (num-val 40) message)
                              (newline message)
                              (let ((result (get-output-string message)))
                                (close-output-port message)
                                result)))))

  (test expval->mutpair:yes
    (begin
      (initialize-store!)
      (expval->mutpair (mutpair-val (a-pair (newref (num-val 41))
                                            (newref (num-val 42))))))
    1 (mutpair?)
    (lambda (result)
      (cases mutpair result
        (a-pair (left-init right-init)
          (and (= (expval->num (deref left-init)) 41)
               (= (expval->num (deref right-init)) 42))))))

  (test expval->mutpair:no
    (guard (object
            ((and (error-object? object)
                  (string=? (error-object-message object) error-message))
             'pass)
...
(suite mutpair? ()
  ...
  (test mutpair?:yes
    (begin
      (initialize-store!)
      (mutpair? (a-pair (newref (num-val 43)) (newref (num-val 44)))))
    1 (true?))
...
(suite make-pair ()
  ...
  (test make-pair:minimal
    (begin
      (initialize-store!)
      (make-pair (num-val 45) (num-val 46)))
    1 (mutpair?)
    (lambda (result)
      (cases mutpair result
        (a-pair (left-loc right-loc)
          (and (= (expval->num (deref left-loc)) 45)
               (= (expval->num (deref right-loc)) 46))))))

  (test make-pair:pair-in-pair
    (begin
      (initialize-store!)
      (make-pair (num-val 47)
                 (mutpair-val (make-pair (num-val 48) (bool-val #f)))))
    1 (mutpair?)
    (lambda (result)
      (cases mutpair result
        (a-pair (left-loc right-loc)
               ...
               (cases mutpair (expval->mutpair (deref right-loc))
                 (a-pair (left-loc right-loc)
                   (and (= (expval->num (deref left-loc)) 48)
                        (false? (expval->bool (deref right-loc))))))))))))
...
(suite left ()
  ...
  (test left:minimal
    (begin
      (initialize-store!)
      (left (make-pair (num-val 50) (num-val 51))))
    1 (expval?)
    (lambda (result)
      (= (expval->num result) 50))))
...
(suite right ()
  ...
  (test right:minimal
    (begin
      (initialize-store!)
      (right (make-pair (num-val 50) (num-val 51))))
    1 (expval?)
    (lambda (result)
      (= (expval->num result) 51))))


(suite setleft ((sample (begin
                          (initialize-store!)
                          (make-pair (num-val 52) (num-val 53)))))

  (test setleft:minimal
    (setleft sample (num-val 54))
    1 ((lambda (ignored) #t))
    (lambda (ignored)
      (cases mutpair sample
        (a-pair (left-loc right-loc)
          (= (expval->num (deref left-loc)) 54))))))


(suite setright ((sample (begin
                           (initialize-store!)
                           (make-pair (num-val 55) (num-val 56)))))

  (test setright:minimal
    (setright sample (num-val 57))
    1 ((lambda (ignored) #t))
    (lambda (ignored)
      (cases mutpair sample
        (a-pair (left-loc right-loc)
          (= (expval->num (deref right-loc)) 57))))))
