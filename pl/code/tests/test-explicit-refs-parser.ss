...
;;; Tests for the (EXPLICIT-REFS parser) library
...
(suite parse-expression ()
  ...
  (test parse-expression:newref-exp
    (parse-expression (scanner (make-character-source "newref(86)")))
    1 (expression?)
    (lambda (result)
      (cases expression result
        (newref-exp (initializer)
          (cases expression initializer
            (const-exp (datum) (= datum 86))
            (else #f)))
        (else #f))))
  ...
  (test parse-expression:deref-exp
    (parse-expression (scanner (make-character-source "deref(waldo)")))
    1 (expression?)
    (lambda (result)
      (cases expression result
        (deref-exp (reference)
          (cases expression reference
            (var-exp (id) (eq? id 'waldo))
            (else #f)))
        (else #f))))

  (test parse-expression:setref-exp
    (parse-expression (scanner (make-character-source "setref(bazola,88)")))
    1 (expression?)
    (lambda (result)
      (cases expression result
        (setref-exp (target source)
          (and (cases expression target
                 (var-exp (id) (eq? id 'bazola))
                 (else #f))
...
(suite parse-program ()
  ...
  (test parse-program:all-in-one
    (parse-program (scanner (make-character-source
                              "if zero?(phi)
                                  then let chi = newref(16)
                                       in begin
                                            setref(chi, deref(chi));
                                            -(phi,20)
                                          end
...
(suite scan&parse ()
  ...
  (test scan&parse:page-105-top
    (scan&parse "let x = newref(0)
  ...
  (test scan&parse:page-105-bottom
    (scan&parse "let g = let counter = newref(0)
                         in proc (dummy)
                             begin
