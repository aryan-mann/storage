...
(define-library (TYPED-OOF type-checker)
  ...
  (begin
    ...
    ;; TYPED-OOF programs are evaluated in an empty initial environment.
    ...
    ;; that mimics the external representation of types in TYPED-OOF programs.
    ...
    ;; the type of a TYPED-OOF program.
    ...
    ;; the type of a TYPED-OOF expression.
    ...
    (define type-of
      (lambda (exp tenv)
        (cases expression exp
          ...
          ;; type checkers left incomplete 
          (fieldref-exp (exp id)
            (let* ((id-type (type-of id tenv))
                   (exp-type (type-of exp tenv)))
              (check-is-subtype! exp-type (class-type 'object) exp)

              (bool-type)))
              
          (fieldset-exp (exp id val)
            (let* ((exp-type (type-of exp tenv))
                   (id-type (type-of id tenv))
                   (val-type (type-of val tenv))))
              (check-if-subtype! exp-type (class-type 'object) exp)
