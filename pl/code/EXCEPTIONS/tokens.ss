...
;;; for lexical tokens of the EXCEPTIONS programming language,
...
;;; EXCEPTIONS has tokens of twenty-four kinds:
;;; "numbers" (actually, numerals),
;;; identifiers, minus signs, open- and close-parentheses,
;;; commas, equals signs,
;;; and the keywords zero?, if, then, else, let, in, proc, letrec, try, catch,
;;; raise, cons, car, cdr, null?, emptylist, and list.

(define-library (EXCEPTIONS tokens)
  (export token? numeral-token minus-sign open-parenthesis comma
          ...
          try-token catch-token raise-token cons-token car-token cdr-token
          null?-token emptylist-token list-token)
  (import (scheme base)
          (utilities eopl))
  (begin
