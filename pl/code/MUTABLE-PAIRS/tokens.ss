...
;;; for lexical tokens of the MUTABLE-PAIRS programming language,
...
;;; MUTABLE-PAIRS has tokens of twenty-four kinds:
...
;;; begin, end, pair, left, right, setleft, and setright.

(define-library (MUTABLE-PAIRS tokens)
  (export token? numeral-token minus-sign open-parenthesis comma
          ...
          set-token begin-token semicolon end-token pair-token left-token
          right-token setleft-token setright-token)
  (import (scheme base)
          (utilities eopl))
  (begin
