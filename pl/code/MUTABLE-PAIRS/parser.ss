...
;;; A parser for the MUTABLE-PAIRS language
...
;;; This library provides a parser for the MUTABLE-PAIRS language
;;; developed by Daniel P. Friedman and Mitchell Wand
;;; in section 4.4 of their book
;;; _Essentials of programming languages_ (third edition).

(define-library (MUTABLE-PAIRS parser)
  ...
  (begin
      ...
      (lambda (token-source)
        ...
        (let ((current (acquire-token token-source)))
          (cases token current
            ...
            (pair-token ()
              (parse-newpair-exp token-source))

            ;; <expression> ::= left ( <expression> )

            (left-token ()
              (parse-left-exp token-source))

            ;; <expression> ::= right ( <expression> )

            (right-token ()
              (parse-right-exp token-source))

            ;; <expression> ::= setleft ( <expression> , <expression> )

            (setleft-token ()
              (parse-setleft-exp token-source))

            ;; <expression> ::= setright ( <expression> , <expression> )

            (setright-token ()
              (parse-setright-exp token-source))))))
      ...
      (lambda (token-source)
        (let ((candidate (acquire-token token-source)))
          (cases token candidate
            ...
            (pair-token ()
              (report-acquire-identifier-error "The keyword pair"))
            (left-token ()
              (report-acquire-identifier-error "The keyword left"))
            (right-token ()
              (report-acquire-identifier-error "The keyword right"))
            (setleft-token ()
              (report-acquire-identifier-error "The keyword setleft"))
            (setright-token ()
              (report-acquire-identifier-error "The keyword setright"))))))
    ...
    ;; parse-newpair-exp : Token-source -> NewpairExp

    (define parse-newpair-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((left-init (parse-expression token-source)))
          (match-and-discard token-source 'comma)
          (let ((right-init (parse-expression token-source)))
            (match-and-discard token-source 'close-parenthesis)
            (newpair-exp left-init right-init)))))
    
    ;; parse-left-exp : Token-source -> LeftExp

    (define parse-left-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
    ...
    ;; parse-right-exp : Token-source -> RightExp

    (define parse-right-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((pair (parse-expression token-source)))
          (match-and-discard token-source 'close-parenthesis)
    ...
    ;; parse-setleft-exp : Token-source -> SetleftExp

    (define parse-setleft-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((target (parse-expression token-source)))
          (match-and-discard token-source 'comma)
          (let ((source (parse-expression token-source)))
            (match-and-discard token-source 'close-parenthesis)
            (setleft-exp target source)))))
    
    ;; parse-setright-exp : Token-source -> SetrightExp

    (define parse-setright-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((target (parse-expression token-source)))
          (match-and-discard token-source 'comma)
