...
;;; A parser for the EXPLICIT-REFS language
...
;;; This library provides a parser for the EXPLICIT-REFS language
;;; developed by Daniel P. Friedman and Mitchell Wand
;;; in section 4.2 of their book
;;; _Essentials of programming languages_ (third edition).

(define-library (EXPLICIT-REFS parser)
  ...
  (begin
      ...
      (lambda (token-source)
        ...
        (let ((current (acquire-token token-source)))
          (cases token current
            ...
            (newref-token ()
              (parse-newref-exp token-source))

            ;; <expression> ::= deref ( <expression> )

            (deref-token ()
              (parse-deref-exp token-source))

            ;; <expression> ::= setref ( <expression> , <expression> )

            (setref-token ()
              (parse-setref-exp token-source))
      ...
      (lambda (token-source)
        (let ((candidate (acquire-token token-source)))
          (cases token candidate
            ...
            (newref-token ()
              (report-acquire-identifier-error "The keyword newref"))
            (deref-token ()
              (report-acquire-identifier-error "The keyword deref"))
            (setref-token ()
              (report-acquire-identifier-error "The keyword setref"))
            (begin-token ()
    ...
    ;; parse-newref-exp : Token-source -> NewrefExp

    (define parse-newref-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((initializer (parse-expression token-source)))
          (match-and-discard token-source 'close-parenthesis)
          (newref-exp initializer))))
    
    ;; parse-deref-exp : Token-source -> DerefExp

    (define parse-deref-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((reference (parse-expression token-source)))
          (match-and-discard token-source 'close-parenthesis)
          (deref-exp reference))))
    
    ;; parse-setref-exp : Token-source -> SetrefExp

    (define parse-setref-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
