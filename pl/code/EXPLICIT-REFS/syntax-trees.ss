...
;;; Syntax trees for EXPLICIT-REFS
...
;;; for abstract syntax trees of the EXPLICIT-REFS programming language,
...
(define-library (EXPLICIT-REFS syntax-trees)
  (export program? a-program
          expression? const-exp diff-exp zero?-exp if-exp var-exp let-exp
          proc-exp call-exp letrec-exp newref-exp deref-exp setref-exp begin-exp)
  (import (scheme base)
          (utilities eopl))
  (begin

    ;; A sample program at the top of page 105
    ;; of _Essentials of Programming Languages_
    ;; shows that EXPLICIT-REFS allows multiple bindings to be introduced
    ...
    ;; The same example shows that EXPLICIT-REFS supports begin-expressions,
    ;; which I have implemented here as well.

    ;; The grammar for EXPLICIT-REFS is as follows:
    ;;
    ;;       <program> ::= <expression>
    ;;    <expression> ::= <numeral>
    ...
    (define-datatype expression expression?
      ...
      (newref-exp (initializer expression?))
      (deref-exp (reference expression?))
      (setref-exp (target expression?)
                  (source expression?))
