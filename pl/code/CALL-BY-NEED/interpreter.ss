...
;;; An interpreter for the CALL-BY-NEED language

;;; John David Stone
;;; Department of Computer Science
;;; Grinnell College

;;; This is an interpreter for the CALL-BY-NEED programming language,
...
(define-library (CALL-BY-NEED interpreter)
  (export run)
  (import (scheme base)
          (utilities eopl)
          (CALL parser)
    ...
    (define value-of
      (lambda (exp env)
        (cases expression exp
          ...
          (var-exp (id)
            (let* ((loc (apply-env env id))
                   (vorth (deref loc)))
              (if (expval? vorth)
                  vorth
                  (let ((val (value-of-thunk vorth)))
                    (setref! loc val)
                    val))))
