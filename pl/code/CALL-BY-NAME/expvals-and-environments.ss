...
;;; Expressed values and environments for CALL-BY-NAME
...
;;; for expressed values of the CALL-BY-NAME programming language,
...
(define-library (CALL-BY-NAME expvals-and-environments)
  (export expval? num-val bool-val proc-val expval->num expval->bool expval->proc
          proc? a-proc thunk? a-thunk environment? empty-env extend-env
  ...
  (begin
    ...
    ;; An expressed value in CALL-BY-NAME
    ...
    ;; When the evaluation of an expression is deferred
    ;; until the value is actually needed for a computation,
    ;; the expression and the environment in which it is to be evaluated
    ;; are stored in a data structure called a thunk.
    ;; In CALL-BY-NAME, a thunk can be stored in a reference
    ;; that is bound to a parameter,
    ;; to be used when the parameter's value is needed.

    (define-datatype thunk thunk?
      (a-thunk (deferred expression?)
               (saved-env environment?)))
    ...
    ;; CALL-BY-NAME programs are evaluated
