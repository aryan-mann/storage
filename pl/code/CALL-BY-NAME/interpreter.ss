...
;;; An interpreter for the CALL-BY-NAME language

;;; John David Stone
;;; Department of Computer Science
;;; Grinnell College

;;; This is an interpreter for the CALL-BY-NAME programming language,
...
(define-library (CALL-BY-NAME interpreter)
  (export run)
  (import (scheme base)
          (utilities eopl)
          (CALL parser)
  ...
  (begin
    ...
    (define value-of
      (lambda (exp env)
        (cases expression exp
          ...
          (var-exp (id)
            (let ((value-or-thunk (deref (apply-env env id))))
              (if (expval? value-or-thunk)
                  value-or-thunk
                  (value-of-thunk value-or-thunk))))
    ...
    ;; The value-of-thunk procedure "thaws" a thunk,
    ;; evaluating the expression in contains
    ;; in the environment that it contains
    ;; and returning the result as an expval.
    ...
    (define value-of-thunk
      (lambda (th)
        (cases thunk th
          (a-thunk (deferred saved-env)
            (value-of deferred saved-env)))))))
