...
;;; A parser for the MINUS language
...
;;; This library provides a parser for the MINUS language
...
(define-library (MINUS parser)
  ...
  (begin
    ...
    ;; parse-minus-exp : Token-source -> MinusExp

    (define parse-minus-exp
      (lambda (token-source)
        (match-and-discard token-source 'open-parenthesis)
        (let ((negand (parse-expression token-source)))
          (match-and-discard token-source 'close-parenthesis)
          (minus-exp negand))))
      ...
      (lambda (token-source)
        (let ((candidate (acquire-token token-source)))
          (cases token candidate
            ...
            (minus-token ()
              (report-acquire-identifier-error "The keyword minus"))))))
